...
namespace Teknik_Servis_Projesi.Formlar
{
    public partial class FrmGelenMesajlar : Form
    {
        ...
        private void FrmGelenMesajlar_Load(object sender, EventArgs e)
        {
            ...
            labelControl16.Text = db.TBLILETISIM.Select(x => x.KONU == "Teşekkür").Count().ToString();
            labelControl14.Text= db.TBLILETISIM.Select(x => x.KONU == "Rica").Count().ToString();
            labelControl12.Text= db.TBLILETISIM.Select(x => x.KONU == "Şikayet").Count().ToString();
